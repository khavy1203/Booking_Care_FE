...
//Huyên: cho trang chủ
const fetchTopClinicHome = (limit) => {
  return axios.get(`/api/v1/top-clinic-home?limit=${limit}`);
};
...
const deleteClinic = async (clinic) => {
  return await axios.delete("/api/v1/clinic/delete", {
    data: { id: clinic.id },
  });
};
...
const getInforClininicOfUserOnPage = async (
  ...
  return await axios.get(
    `/api/v1/clinic-page?page=${page}&limit=${limit}&province=${province}&district=${district}&ward=${ward}`
  );
};
...
const fetchAllClinicsOfSupport = async (page, limit) => {
  return await axios.get(
    `/api/v1/clinic/fetchAllClinicsOfSupport?page=${page}&limit=${limit}`
  );
};
